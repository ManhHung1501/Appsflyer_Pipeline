...
with DAG(
    ...
    exclude_file = ["__init__.py", "af_registration.py"]
    python_files = [f for f in os.listdir(appsflyer_tasks) if f.endswith(".py") and f not in exclude_file]
    game_task_groups = []
    ...
    for game in game_config:
        ...
        with TaskGroup(group_id = f"Transform_Appsflyer_data_{game_code}") as task_group:
            generate_user_registration_task = SparkSubmitOperator(task_id='generate_user_registration_task',
                                                        conn_id='spark',
            ...
            with TaskGroup(group_id=f"group_data_Appsflyer_API_android") as task_group_API_android:
                ...
                for python_file in python_files:
                    ...
                    spark_submit_task = SparkSubmitOperator(task_id=task_id,
                                                        conn_id='spark',
                                                        application=f"{appsflyer_tasks}/{python_file}",
                                                        application_args=[
                                                            config_dumps,
                                                            'android',
                                                            "{{ ti.xcom_pull(task_ids='load_config', key='start_date') }}",
