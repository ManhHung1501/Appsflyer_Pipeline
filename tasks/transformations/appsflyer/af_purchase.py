...
from utils.common_utils import rename_columns, cast_columns_to_string,get_user_id_for_all, get_server_id_for_all, parse_server_name_by_platform, \
    parse_mobile_carrier_by_platform, get_character_id_for_all, parse_nested_json_udf
...
def trans_af_purchase(config:dict, platform: str, date: str, game_code: str):
    ...
    # Init spark session
    spark = create_spark_s3_session(f"Transform Event Appsflyer Purchase game {game_code} platform {platform} on {date}", 
                                    s3_endpoint,
                                    s3_access_key,
                                    s3_secret_key)
    parse_nested_json_udf
    ...
    for type in ["organic", "non-organic"]:
        ...
        data_frame = data_frame.filter(
            col("Event Name").isin("af_start_iap", "af_pay_fail", "af_pay_success", "af_pay_notyet_success",
                            "af_h5_pay_notyet_success", "af_h5_start_iap", "af_h5_pay_success")
                        )
...
if __name__ == "__main__":
    ...
    trans_af_purchase(config, platform, date, game_code)