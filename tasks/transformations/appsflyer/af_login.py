...
from utils.common_utils import rename_columns, cast_columns_to_string,get_user_id_for_all, get_server_id_for_all, parse_server_name_by_platform, \
    parse_mobile_carrier_by_platform, get_character_id_for_all, parse_login_method_by_platform
...
def trans_af_login(config:dict, platform: str, date: str, game_code: str):
    ...
    for type in ["organic", "non-organic"]:
        ...
        # Transform data
        data_frame = data_frame.filter(col("Event Name").isin("af_login", "af_login_fail", "af_web_login", "af_h5_login_fail"))
        df_renamed = rename_columns(data_frame)
        col_cast_string = ["campaign_id", "is_retargeting", "dataset_id", "event_revenue", "install_time", "event_time"]
        ...
        df_final  = (df_casted_string.withColumn("dataset_id", col("dataset_id").cast(StringType()))
                      ...
                      .withColumn("login_method", parse_login_method_by_platform(col("event_value"), lit(platform)))
                      .fillna(""))
        # Save data to clickhouse
        logging.info(f'Loading data to Clickhouse ...')
        load_data_to_clickhouse(df=df_final,target_db=f"da_cdp_{game_code}", target_tbl="appsflyer_login")
...
if __name__ == "__main__":
    ...
    trans_af_login(config, platform, date, game_code)