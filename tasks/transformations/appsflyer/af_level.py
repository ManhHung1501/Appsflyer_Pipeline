...
def trans_af_level(config:dict, platform: str, date: str, game_code: str):
    ...
    # Init spark session
    spark = create_spark_s3_session(f"Transform Event Appsflyer Level game {game_code} platform {platform} on {date}", 
                                    s3_endpoint,
                                    s3_access_key,
                                    s3_secret_key)
    ...
    for type in ["organic", "non-organic"]:
        ...
        data_frame = data_frame.filter(col("Event Name").rlike("^af_lev_"))
        ...
        df_casted_string = cast_columns_to_string(df_renamed, col_cast_string).withColumnRenamed("adset_id", "dataset_id")
        ...
        load_data_to_clickhouse(df=df_final,target_db=f"da_cdp_{game_code}", target_tbl="appsflyer_level")
...
if __name__ == "__main__":
    ...
    trans_af_level(config, platform, date, game_code)