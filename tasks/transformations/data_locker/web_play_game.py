...
def trans_af_web_play_game(config:dict, date: str, game_code: str):
    ...
    # Init spark session
    spark = create_spark_s3_session(f"Transform Event Appsflyer Web Play Game game {game_code} on {date}", 
                                    s3_endpoint,
                                    s3_access_key,
                                    s3_secret_key)
    ...
    # Transform data
    data_frame = data_frame.drop("_c0").filter(col("event_name").isin("af_h5_play_game"))
    ...
    load_data_to_clickhouse(df=df_final,target_db=f"da_cdp_{game_code}", target_tbl="appsflyer_web_play_game")
...
if __name__ == "__main__":
    ...
    trans_af_web_play_game(config, date, game_code)