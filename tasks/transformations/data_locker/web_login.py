...
def trans_af_web_login(config:dict, date: str, game_code: str):
    ...
    # Init spark session
    spark = create_spark_s3_session(f"Transform Event Appsflyer Web Login game {game_code} on {date}", 
                                    s3_endpoint,
                                    s3_access_key,
                                    s3_secret_key)
    ...
    # Transform data
    data_frame = data_frame.drop("_c0").filter(col("event_name").isin("af_login", "af_login_fail", "af_web_login", "af_h5_login_fail"))
    col_cast_string = ["customer_user_id", "event_time",]
    ...
    df_final  = (df_casted_string.withColumn("user_id", parse_nested_json_udf(col("event_value"), lit("af_fid")))
                ...
                .withColumn("login_method", parse_nested_json_udf(col("event_value"), lit("af_login_method")))
    ...
    load_data_to_clickhouse(df=df_final,target_db=f"da_cdp_{game_code}", target_tbl="appsflyer_web_login")
...
if __name__ == "__main__":
    ...
    trans_af_web_login(config, date, game_code)