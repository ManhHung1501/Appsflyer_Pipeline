...
def trans_af_web_tutorial_complete(config:dict, date: str, game_code: str):
    ...
    # Init spark session
    spark = create_spark_s3_session(f"Transform Event Appsflyer Web Tutorial Complete game {game_code} on {date}", 
                                    s3_endpoint,
                                    s3_access_key,
                                    s3_secret_key)
    ...
    # Transform data
    data_frame = data_frame.drop("_c0").filter(col("event_name").isin("af_h5_tutorial_completed"))
    col_cast_string = ["customer_user_id"]
    ...
    load_data_to_clickhouse(df=df_final,target_db=f"da_cdp_{game_code}", target_tbl="appsflyer_web_tutorial_complete")
...
if __name__ == "__main__":
    ...
    trans_af_web_tutorial_complete(config, date, game_code)